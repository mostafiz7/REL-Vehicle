...
class Purchase_Controller extends Controller
{
  ...
  public function VehiclePartsPurchase_Index( Request $request )
  {
    ...
    if( ! empty($country_origin) ){
      $purchases_all = $purchases_all->whereRelation('details', 'origin', '=', $country_origin);
    }
  ...
  {
    // if( Gate::allows('isAdmin', Auth::user()) ){}
    if( Gate::denies('isSuperAdmin') || Gate::denies('entryDelete') || Gate::denies('routeHasAccess') ){
      return back()->with('error', 'You are not authorized to perform this action!');
    }
  ...
  {
    // if( Gate::allows('isAdmin', Auth::user()) ){}
    if( Gate::denies('isAdmins') || Gate::denies('entryDelete') || Gate::denies('routeHasAccess') ){
      return back()->with('error', 'You are not authorized to perform this action!');
    }
