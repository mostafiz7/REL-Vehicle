          ...
          <div class="parts-all-area full-height-parent">
            ...
            <div class="parts-all-details overlay-scrollbar full-height-minus minus-70 p-10">
              @if ( $parts_all && count($parts_all) > 0 )
                {{-- Paginate-Links --}}
                <div class="pagination-links {{ $parts_all->total() > $pagination_count ? 'mb-5' : '' }} {{ $parts_all->currentPage() == 1 ? 'first-page' : ($parts_all->currentPage() == $parts_all->lastPage() ? 'last-page' : '') }}">
                  {{ $parts_all->withQueryString()->links() }}
                </div>
                ...
                <table class="table table-bordered table-hover border-secondary-3 parts-all-table">
                  ...
                  @php
                    $serial = $parts_all->currentPage() == 1 ? 1 : ((($parts_all->currentPage() - 1) * $pagination_count) + 1);
                  @endphp

                  <tbody class="table-body fz-12">
                    @foreach ( $parts_all as $index => $parts )
                      @include('modules.vehicle-module.parts.index-tableRow', ['parts' => $parts, 'serial' => $serial++])
                    @endforeach
                  </tbody>
                </table>

                {{-- Paginate-Links --}}
                <div class="pagination-links {{ $parts_all->total() > $pagination_count ? 'my-20' : '' }} {{ $parts_all->currentPage() == 1 ? 'first-page' : ($parts_all->currentPage() == $parts_all->lastPage() ? 'last-page' : '') }}">
                  {{ $parts_all->withQueryString()->links() }}
                </div>

              @else
                <div class="no-parts text-danger align-middle fz-22 fw-bold text-center py-100">
                  Sorry! Currently there are no parts available.
                </div>
              @endif
